...
const MagnifierComponent = ({ context, onChange }: IDropperProps) => {
  ...
  const { matrixRepresantationData, clientRect } = useMemo(() => {
    if (context) {
      ...
      const matrixRepresantationData = convertInto2DMatrix(
        imageData,
        context.width * 4,
      );
      return {
  ...
  useEffect(() => {
    ...
    const onMouseMove = (event: MouseEvent) => {
      const { top: boundTop, left: boundLeft, width, height } = clientRect;
    ...
    const onClick = (e: Event) =>
      onChange ? onChange(centralColorRef.current, true, e) : null;
