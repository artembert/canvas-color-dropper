...
const needBorder = (x: number, y: number) => x === centralIndex && y === centralIndex;
const rgbToHex = (r: number, g: number, b: number): string => ((r << 16) | (g << 8) | b).toString(16);
...
const getColorsMatrix = (imageData: number[][], x: number, y: number, widthRatio: number, heightRatio: number) => {
    const nextColors = [];
